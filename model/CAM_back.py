...
def show_cam_on_image_plt(img, mask, file_path="cam.jpg", raw_path=None):
    ...
    cam = 0.8*cv2.resize(heatmap,(224,224)) +  img
...
def generate_mask_BB_max_area_center(mask, bird_BB, scale=4, resize_WH=False, out_of_edge=False):
    """
...
def get_IoU_Image(idx, imgs, maps, save_dir, save_att_idx, names, groups, attri_names, KPs, bird_BB, scale , resize_WH, out_of_edge, max_area_center):
    ...
    for group_name, group_dims in groups.items():
        ...
        if group_name != 'others':
            ...
            # KP_BB = KP_BBs[part_id]
            # KP_BB_dict = {'x1': KP_BB[0], 'x2': KP_BB[2], 'y1': KP_BB[1], 'y2': KP_BB[3]}
            ...
            for group_dim in group_dims:
                for j, name in enumerate(names):
                    ...
                    if max_area_center:
                        mask_BB_dict, (mask_c_x, mask_c_y) = generate_mask_BB_max_area_center(mask, bird_BB, scale, resize_WH, out_of_edge)
                    else:
                        mask_BB_dict, (mask_c_x, mask_c_y) = generate_mask_BB(mask, bird_BB, scale)
                    # mask_BB_dict = generate_mask_BB_mass_center(mask, bird_BB, scale)
                    # print("mask_BB_dict:", mask_BB_dict)
                    # time_2 = time.time()
                    # print('sub time for generate mask:', time_2 - time_1)
                    # find the closest point to the mask center (mask_c_x, mask_c_y)
                    ...
                    if KP_M_dis_best == 1000:
                        ...
                        # time_3 = time.time()
                        # print('sub time for calculate IoU:', time_3 - time_1)
                        # print('IoU:', IoU)

                        if save_dir and save_att_idx[idx] == 1:

                        # if save_dir and save_att_idx[idx] == 1 and IoU > 0.5:
...
def calculate_atten_IoU(input, impath, save_att_idx, maps, names, vis_groups, KP_root=None, save_att=False, scale=4, resize_WH=False, out_of_edge=False, max_area_center=False):
    """
    ...
    for i, path in enumerate(impath):
        ...
        img_IoU = get_IoU_Image(i, img_raw_show, maps, save_dir, save_att_idx, names, vis_group, attri_names, KPs, bird_BB, scale, resize_WH, out_of_edge, max_area_center)
        time_3 = time.time()
...
def show_cam_on_image(img, mask, file_path="cam.jpg", raw_path=None):
    ...
    #cv2.imwrite(file_path,np.uint8(255 * img))  # grad-cam图像
    mask_max = np.max(mask)
    mask_min = np.min(mask)
    mask = (mask - mask_min) / (mask_max - mask_min)
    heatmap = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)

    cv2.imwrite(file_path, np.uint8( cv2.resize(heatmap,(224,224))))

    heatmap = np.float32(heatmap) / 255
    #img = np.transpose(img, (1, 2, 0))
    img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    cv2.imwrite(file_path, np.uint8(255*img))
    cam = cv2.resize(np.float32(heatmap),(224,224))  +  np.float32(img)

    cam = cam / np.max(cam)
    cv2.imwrite(file_path, np.uint8(255 * cv2.resize(cam,(224,224))))  # grad-cam图像
