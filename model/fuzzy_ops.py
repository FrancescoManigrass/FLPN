    ...
    def __call__(self, xs,p=2.0, dim=None, keepdim=False, mask=None):
        ...
        if mask is not None:
            ...
            if not hasattr(xs, 'weight'):
                return torch.div(torch.sum(torch.mul(torch.tensor(1.0).cuda(), xs)),torch.sum(torch.tensor(1.0).cuda()))
            return torch.div(torch.sum(torch.mul(torch.tensor(xs.weight).cuda(),xs)),torch.sum(torch.tensor(xs.weight).cuda()))
