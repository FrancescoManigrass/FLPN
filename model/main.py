...
def main():
    ...
    def create_model(attribute, binary_att, data, original_att):
        ...
        if opt.model == "resnet101":
            ...
            model =  ViT(opt=opt,attribute=attribute,binary_att= binary_att, data=data, original_att=original_att,model_name='vit_large_patch16_224_in21k',pretrained=True)
        ...
        for epoch in range(opt.nepoch):
            ...
            if not opt.resume and epoch < opt.pretrain_epoch:  # pretrain ALE for the first several epoches
                ...
                if opt.optimizer == "sgd":
                    optimizer = optim.SGD(params=filter(lambda p: p.requires_grad, parameters), lr=opt.pretrain_lr,
            ...
            class_attribute_original = data.binary_att
            ...
            attribute_zsl_original = prepare_attri_label(class_attribute_original, data.unseenclasses)
            attribute_seen_original = prepare_attri_label(class_attribute_original, data.seenclasses)
            attribute_macroclass_seen = prepare_attri_label(data.attribute_macroclass, data.seen_macroclass)
            ...
            if True:
                ...
                for i, (batch_input, batch_input2, batch_target, impath) in tqdm(enumerate(trainloader),
                    ...
                    if opt.cuda:
                        input_v = input_v.to(opt.device)
                        label_v = label_v.to(opt.device)
                        label_m = label_m.to(opt.device)

                    axioms_options = {"p_axioms_class": 4.0, "p_axioms_macroclass": 1.0,
                                      "p_axioms_class_exists": 1.0, "p_axioms_class_cluster": 1.0, "p_all": 2.0}

                    if epoch >= 4:
                        axioms_options = {"p_axioms_class": 6.0, "p_axioms_macroclass": 2.0,
