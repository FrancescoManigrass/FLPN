...
from visual_utils import ImageFilelist,ImageFilelist2, compute_per_class_acc, compute_per_class_acc_gzsl, \
...
class CategoriesSampler():
    ...
    def __init__(self, label_for_imgs, batch_size=1000, n_cls=16, n_per=3, ep_per_batch=1):
        self.batch_size = batch_size # batchs for each epoch self.n_batch
        self.n_cls = n_cls # ways
        self.n_per = n_per # shots
        self.ep_per_batch = ep_per_batch # episodes for each batch, defult set 1
...
def test_zsl(opt, model, testloader, attribute, test_classes,attribute_original):
        ...
        for i, (input, target, impath) in \
            ...
            if opt.cropped_image:
                ...
                for i in range(len(boxes)):
                    cropped = torchvision.transforms.functional.crop(input[i, :, :, :], boxes[i, 0], boxes[i, 1],
                                                                     boxes[i, 2] - boxes[i, 0],
                                                                     boxes[i, 3] - boxes[i, 1])

                    cropped = fn.resize(cropped, size=[input[i, :, :, :].shape[1], input[i, :, :, :].shape[2]])
            ...
            output = model.calculate_axioms(pre_attri, attribute=attribute, opt=opt,get_prediction=True,labels_test=test_classes,
                                            original_attribute=attribute_original)
    ...
    acc_all, acc_avg = compute_per_class_acc(visual_utils.map_label(torch.from_numpy(GT_targets), test_classes).numpy(),np.array(predicted_labels), test_classes.numpy())
    """
...
def test_gzsl(opt, model, testloader, attribute, test_classes,all_classes,original_att):
    #layer_name = model.extract[0]
    GT_targets = []
    predicted_labels = []
    predicted_layers = []
    ...
    with torch.no_grad():
        for i, (input, target, impath) in \
            ...
            if opt.cropped_image:
                cropped_images= crop_images(input,boxes,opt)
                ...
                if opt.print_images:
                    ...
                    for k in range(len(cropped_images)):
                        ...
                        image = Image.open(
                            opt.image_root + "/" + opt.dataset + "/" + "JPEGImages" + "/" + folder + "/" + folder + "_" + name_image)

                        image = image.resize((224, 224))

                        cropped=image.crop((int(boxes[0][0]), int(boxes[0][1]), int(boxes[0][2]), int(boxes[0][3])))
                        cropped.save(f'output_images/{folder}/{folder}_{name_image}')
            ...
            output = model.calculate_axioms(pre_attri, attribute=attribute, opt=opt, get_prediction=True,
                                            labels_test=all_classes,original_attribute=original_att)
    ...
    acc_all, acc_avg = compute_per_class_acc_gzsl(GT_targets,
                                     np.array(predicted_labels), test_classes.numpy()) ### 测试的时候用的全局分支 校正后的结果
    """
...
def test_gzsl_loss(opt, model, testloader, attribute,reg_weight,criterion, criterion_regre,realtrain,middle_graph,parts,group_dic,sub_group_dic):

    loss_log = {'ave_loss': 0, 'contrastive': 0, 'l_xe_final': 0, 'l_attri_final': 0, 'l_regular_final': 0,
                'l_xe_layer': 0, 'l_attri_layer': 0, 'l_regular_layer': 0, 'l_cpt': 0}
    batch = len(testloader)
    ...
    with torch.no_grad():
        for i, (input, target, impath) in \
            ...
            loss = Loss_fn(opt, loss_log, reg_weight, criterion, criterion_regre, model,
                            output, pre_attri, attention, pre_class, label_a, target,
                            realtrain, middle_graph, parts, group_dic, sub_group_dic)


            loss_contrastive = criterion_contras(pre_attri[layer_name],target)
            loss += loss_contrastive

            loss_log['ave_loss'] += loss.item()
            loss_log['contrastive'] += loss_contrastive.item()
    return loss_log['ave_loss'] / batch , loss_log['contrastive'] / batch
...
def get_loader(opt, data):
    ...
    dataset_train = ImageFilelist2(opt, data_inf=data,
...
def Loss_fn(opt, loss_log, reg_weight, criterion, criterion_regre, model,
    ...
    if reg_weight['final']['xe'] > 0:
        loss_xe = reg_weight['final']['xe'] * criterion(output, label_v)  #全局分支分类loss
        loss_log['l_xe_final'] += loss_xe.item()
        loss = loss_xe

    if reg_weight['final']['attri'] > 0:
        loss_attri = reg_weight['final']['attri'] * criterion_regre(pre_attri['final'], label_a) #全局分支回归Loss
    ...
    if opt.additional_loss:
        ...
        for name in model.extract:  # "name" is layer4 currently
            if reg_weight[name]['l_xe'] > 0:
                layer_xe = reg_weight[name]['l_xe'] * criterion(pre_class[name], label_v) #局部分支分类loss
                loss_log['l_xe_layer'] += layer_xe.item()
                loss += layer_xe

            if reg_weight[name]['attri'] > 0:
                loss_attri = reg_weight[name]['attri'] * criterion_regre(pre_attri[name], label_a) #局部分支回归loss
            ...
            if opt.use_group:
                ...
                if reg_weight[name]['cpt'] > 0 and realtrain:
                    ...
                    loss += cpt_loss                          #CPTloss

                for part in parts[:7]:
                    group = group_dic[part]
                    # res_loss
                    if reg_weight[name]['regular'] > 0:
                        reg_loss = reg_weight[name]['regular'] * add_glasso(weight_layer, group)   # 原型AD loss
                        loss_log['l_regular_layer'] += reg_loss.item()
                        loss += reg_loss

                    if reg_weight['final']['regular'] > 0:
                        reg_loss = reg_weight['final']['regular'] * add_glasso(weight_final, group) # 全局分支 参数 V 的AD loss
...
class SupConLoss_clear(nn.Module):  #### contrastive loss
    def __init__(self, temperature=0.07):
        super(SupConLoss_clear, self).__init__()
        self.temperature = temperature
    ...
    def forward(self, features, labels):
        ...
        mask = mask * logits_mask #stessa classe * non stessa immagine -> prende solo gli elementi che sono della stessa classe ma non sono la stessa immagine
        single_samples = (mask.sum(1) == 0).float() # immagini che non hanno un altro elemento della stessa classe

        # compute log_prob
        exp_logits = torch.exp(logits) * logits_mask #probabilità dei logits che non sono la stessa immagine ( positivi e negativi) ovvero il denominatore che mi serve
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True)) # sommo i logits delle immagini che sono della stessa classe ma non sono la stessa immagine e poi li sottraggo al totale
        # differenza logaritivo = divisione tra gli exp quindi autoamenticamente sta facendo la divisione per ogni elemento ottenendo la matrice delle divisioni praticamente

        # compute mean of log-likelihood over positive
        # invoid to devide the zero
        mean_log_prob_pos = (mask * log_prob).sum(1) / (mask.sum(1)+single_samples) # di tutta la matrice mi prendo solo i casi dei positivi iniziali e li divido per quamti sono
...
def mse_loss(input_logits, target_logits):
    ...
    return loss_fn(input_logits, target_logits)
...
def get_current_consistency_weight(opt, epoch):
