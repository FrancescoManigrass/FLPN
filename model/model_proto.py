...
class resnet_proto_IoU(nn.Module):
    def __init__(self, opt, attribute, binary_att, data, original_att):
        ...
        if opt.cuda:
            for f in range(len(self.hasAttribute)):
                self.hasAttribute[f]=self.hasAttribute[f].to(torch.device("cuda:"+opt.gpu))
        """
        """
    ...
    def calculate_axioms(self, pre_attri, attribute, label=False, label_m=None, attribute_macroclass_seen=None,
        ...
        if opt.cropped_image:
            ...
            visual_score_class = torch.einsum('bd,nd->bn', F.normalize(pre_attri['final'], dim=-1), prototypes_class) * self.scale
        ...
        if get_prediction == False:
            ...
            index_of_attribute = torch.masked_select(
                torch.arange(self.part_num).unsqueeze(0).repeat(visual_score.shape[0], 1).view(-1, 1).cuda(), mask).view(-1, 1)
            ...
            if opt.sat_agg_same_attribute and attribute_features.value.shape[0] != 0:
            ...
            if not opt.logltn:
                ...
                return -torch.log((self.SatAgg(my_dict.values(),
