...
class ProtoModel(nn.Module):
    ...
    def forward(self, attrs, is_cls="is_cls"):
        if is_cls == "is_cls":
            protos = self.relu3(self.fc3(self.cn2(self.relu2(self.cn1(self.fc2(self.relu1(self.fc1(attrs))))))))
        elif is_cls == "is_att":
            protos = self.relu3(
                self.fc3_att(self.cn2_att(self.relu2(self.cn1_att(self.fc2(self.relu1(self.fc1(attrs))))))))
        elif is_cls == "is_macro":
            protos = self.relu3(
                self.fc3_macro(self.cn2_macro(self.relu2(self.cn1_macro(self.fc2(self.relu1(self.fc1(attrs))))))))
        else:
            print("error in forward of prototype")
            exit(-1)
        return protos
...
def euclid_distance(x, y,p):
    x = torch.nn.functional.normalize(x, p=2, dim=1)
    y = torch.nn.functional.normalize(y, p=2, dim=1)
    #dist = 1. - x @ y.T
    return torch.exp(-p*torch.norm(x-y,dim=1))
