...
class DATA_LOADER(object):
    def __init__(self, opt):
        ...
        self.attribute_macroclass = None
        if opt.matdataset:
    ...
    def read_matdataset(self, opt):
        ...
        print("loading from",opt.dataroot + "/" + opt.dataset + "/" + opt.image_embedding + ".mat")
        ...
        if opt.dataset != "CUB":
            ...
            matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.class_embedding + "_splits.mat")
            f = open(opt.dataroot + "/" + opt.dataset + "/" + "images.txt", "r")
            self.image_files = []
            for f1 in f:
                self.image_files.append(f1.split()[1])
        ...
        classes_name = [f[0][0] for f in self.allclasses_name]
        self.attribute = torch.from_numpy(matcontent['att'].T).float()
        self.original_att = torch.from_numpy(matcontent['original_att'].T).float()
        self.binary_att = torch.from_numpy(matcontent['binary_att'].T).int()
        self.attri_name = matcontent['attri_name']

        with open(join(opt.dataroot, "Attribute", "w2v", opt.dataset + "_attribute.pkl"), 'rb') as f:
            data = pickle.load(f)
        self.w2v = data

        if opt.dataset == "CUB" or opt.dataset == "AWA2":
            file = open(opt.dataroot + "/" + opt.dataset + "/" + "wordnet" + opt.dataset + ".pickle", 'rb')
            wordnet = pickle.load(file)
        if opt.dataset == "CUB":
            wordnet = OrderedDict((k, wordnet[k.split(".")[1].lower()]) for k in classes_name)
            wordnet = OrderedDict((k, wordnet[k]) for k in classes_name)
            json_file = open(opt.dataroot + "/" + opt.dataset + "/" + 'attri_groups_8.json')
            ...
            for f in range(self.attribute.shape[1]):
                for key, item in data.items():
                    if f in item:
                        self.parts_key.append(keys.index(key))

            self.parts_key = torch.from_numpy(np.array(self.parts_key)).view(-1, 1)
            self.parts_names = keys
            ...
            for i in range(len(keys)):
                ...
                for j in range(matcontent['original_att'].shape[0]):
                    # Verifica se j è contenuto nella lista associata alla chiave i
                    if j in data[list(data.keys())[i]]:
                        row.append(1)
                    else:
                        row.append(0)

                self.parts.append(row)
            self.parts = torch.from_numpy(np.array(self.parts))


        if opt.dataset == "AWA2":
            wordnet = OrderedDict((k, wordnet[k]) for k in classes_name)
            json_file = open(opt.dataroot + "/" + opt.dataset + "/" + 'attri_groups_9.json')
            data = json.load(json_file)
            self.parts = []
            self.parts_key = []
            self.parts_data = data
        ...
        if opt.dataset == "CUB" or opt.dataset == "AWA2":
            file.close()
            categories = sorted(list(set([wordnet[f]['macroclass'] for f in wordnet])))

            d = {}
            for f1 in [wordnet[f]["macroclass"] for f in wordnet]:
                d[f1] = []
            for f in wordnet:
                d[wordnet[f]["macroclass"]].append(f)

            self.wordnet = d
            self.wordnet_conversion = {}

            self.allmacroclass_name = list(self.wordnet.keys())
            self.all_macroclass = []
            for c in range(len(self.allclasses_name)):
                for key, item in enumerate(self.wordnet):
                    if self.allclasses_name[c][0][0] in self.wordnet[item]:
                        self.all_macroclass.append(key)
                        self.wordnet_conversion[c] = key

            macroclass_attribures = {f: [] for f in range(0, len(list(set(self.all_macroclass))))}

            for f in range(len(self.all_macroclass)):
                macroclass_attribures[self.all_macroclass[f]].append(self.binary_att[f])

            attribute_macroclass = []
            for f in macroclass_attribures:
                attribute_macroclass.append(torch.stack(macroclass_attribures[f], dim=0).sum(dim=0))

            self.attribute_macroclass = torch.where(torch.stack(attribute_macroclass, dim=0) > 0, 1, 0).float()
        ...
        self.unseenclasses = torch.from_numpy(np.unique(self.test_unseen_label.numpy()))
        self.seen_macroclass = []
        if opt.dataset == "CUB" or opt.dataset == "AWA2":
            for f in range(len(list(self.wordnet_conversion.keys()))):
                if list(self.wordnet_conversion.keys())[f] in self.seenclasses.numpy():
                    self.seen_macroclass.append(self.all_macroclass[f])
            self.seen_macroclass = torch.as_tensor(np.unique(np.array(self.seen_macroclass)), dtype=torch.long)
    ...
    def next_batch(self, batch_size):
        ...
        batch_original_att = self.original_att[batch_label]
        return batch_feature, batch_label, batch_att, batch_original_att
...
def default_flist_reader(opt, image_files, img_loc, image_labels, dataset, wordnet_conversion=None):
    """
    flist format: impath label\nimpath label\n ...(same to caffe's filelist)
    """
    imlist = []
    image_files = np.array(image_files)[img_loc.astype(int)]
    image_labels = image_labels[img_loc]
    for image_file, image_label in zip(image_files, image_labels):
        if dataset == 'CUB':
            image_file = opt.image_root + "/CUB/images/" + image_file
        elif dataset == 'AWA1':
            ...
            exit(1)
        if wordnet_conversion:
            # if wordnet_conversion[int(image_label)] ==6:
            ## print("error")
            imlist.append((image_file, int(image_label), wordnet_conversion[int(image_label)]))
        else:
...
class ImageFilelist2(torch.utils.data.Dataset):
    def __init__(self, opt, data_inf=None, transform=None, transform2=None, target_transform=None, dataset=None,
                 flist_reader=default_flist_reader, loader=default_loader, image_type=None, select_num=None):
        self.transform = transform
        ...
        self.image_labels = data_inf.label
        self.dataset = dataset
        if self.dataset == "CUB" or self.dataset == "AWA2":
            self.wordnet_conversion = data_inf.wordnet_conversion
            self.imlist = flist_reader(opt, self.image_files, self.img_loc, self.image_labels, self.dataset,
                                       self.wordnet_conversion)
        else:
            self.imlist = flist_reader(opt, self.image_files, self.img_loc, self.image_labels, self.dataset)
    ...
    def __getitem__(self, index):
        ...
        # print(self.imlist[index])
        # index = index.int()
        if len(self.imlist[index]) == 2:
            impath, target = self.imlist[index]
        else:
            impath, target, target_macroclass = self.imlist[index]
        img = self.loader(impath)
        img2 = self.loader(impath)
        if self.transform is not None:
            img = self.transform(img)
        ...
        if self.target_transform is not None:
            target = self.target_transform(target)
        if len(self.imlist[index]) == 3:

            # print(img, img2, (target,target_macroclass), impath)

            return img, img2, (target, target_macroclass), impath
        else:
            return img, img2, target, impath
...
def save_predict_txt(output, predict_txt_path, attri, predicted_label, target, class_attribute, attri_names,
                     class_names):
    ...
    file.write(
        'predict attri_name for {}, '.format(class_names[predicted_label][0][0]) + 'the value is {}:\n \n'.format(
            sum(predict_att_mul)))
...
def add_paths(opt, img_path, correct, predicted_label, attri_name, allclasses_name, attri_id=None, attri_rank=None,
              attri_weight=None, dataset=None):
    if dataset == "CUB":
    ...
    final_vis_path = os.path.join(final_vis_path, correct + "_" + allclasses_name[predicted_label][0][
        0] + "_" + final_vis_name).replace('_{}.png'.format(attri_id), '_all.png')
    vis_name = img_path.split('/')[1].replace('.jpg', '_rank{}_{}_weight{:.3}.jpg'.format(attri_rank,
                                                                                          attri_name[attri_id].strip(),
                                                                                          attri_weight))
    ...
    predict_txt_path = os.path.join(predict_txt_path, correct + "_" + allclasses_name[predicted_label][0][
        0] + "_" + final_vis_name.replace('.jpg', '.txt'))
...
def add_image_attri_L2_path(opt, img_path, correct, predicted_label, attri_name, allclasses_name, attri_id=None,
                            attri_rank=None, attri_weight=None, attri_dis=None, dataset=None):
    ...
    mask_path = os.path.join(opt.image_root,
                             'visual/{}/{}/masks/{}'.format(opt.vis_type, opt.train_id, img_path.split('/')[0]))
    ...
    final_vis_name = img_path.split('/')[1].replace('.jpg', '_all.jpg')
    # # print('attri:', attri_name[attri_id].strip(), len(attri_name[attri_id]))
    final_vis_path = os.path.join(final_vis_path,
                                  correct + "_" + allclasses_name[predicted_label] + "_" + final_vis_name).replace(
        '_{}.png'.format(attri_id), '_all.png')
    vis_name = img_path.split('/')[1].replace('.jpg', '_rank{}_{}_dis{:.3}_weight{:.3}.jpg'.format(attri_rank,
                                                                                                   attri_name[
                                                                                                       attri_id].strip(),
                                                                                                   attri_dis,
                                                                                                   attri_weight))
    ...
    raw_path = os.path.join(raw_path,
                            "{}_pred_{}_GT_{}".format(correct, allclasses_name[predicted_label], final_vis_name))
    # # print("vis_path:", vis_path)
...
def find_contours(mask_path, newshape=None, side=None):
    global image_cnts_extra
    mask = cv2.imread(mask_path)

    if newshape != None:
        if side != None and side == "L":
            mask = cv2.flip(mask, 1)
        mask = cv2.resize(mask, (newshape[0], newshape[1]))

    img_gray = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY)
    # ret, im = cv2.threshold(img_gray, 100, 255, cv2.THRESH_BINARY)

    ret, im = cv2.threshold(img_gray, 0, 255, cv2.THRESH_BINARY)

    cnts, hierarchy = cv2.findContours(im, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
