    ...
    def calculate_axioms(self, pre_attri, attribute, label=False, label_m=None, attribute_macroclass_seen=None,
        ...
        if get_prediction == False:
            ...
            prototypes_class_fake = self.proto_model(
                create_fake(F.normalize(attribute.T, dim=-1) * np.sqrt(self.part_num)), "is_cls")
            ...
            features=torch.arange(85).unsqueeze(0).repeat(visual_score.shape[0], 1).view(-1, 1).to(torch.device("cuda:"+opt.gpu))
